...
# 使用以下命令直接执行
# CUDA_VISIBLE_DEVICES=0,1,2,3 python -m torch.distributed.launch --nproc_per_node=4 Fire_rate.py
...
for epoch in range(num_epochs):
        ...
        if ((i+1)<=len(train_dataset)//batch_size):
            ...
            if (i+1)%10 == 0:
                if master : 
                    ...
                    fire_rate_list.append(fire_rate.item())
                    writer.add_scalar('fire_rate', fire_rate.item(), training_iter)
    ...
    #evaluation
    #val(optimizer,snn,test_loader,test_dataset,batch_size,epoch)
    optimizer = lr_scheduler(optimizer, epoch+start_epoch, 0.3, 15)
