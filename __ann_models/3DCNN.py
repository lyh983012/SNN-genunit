...
def resnet18(pretrained=False, progress=True, **kwargs):
    return _resnet('resnet18', Bottleneck, [3, 2, 2, 2], pretrained, progress,
                   **kwargs)

def conv3x3(in_planes, out_planes, stride=1, groups=1, dilation=1):
    """3x3 convolution with padding"""
    return nn.Conv3d(in_planes, out_planes, kernel_size=[3,3,3], stride=stride,
                     padding=[dilation,dilation,1], groups=groups, bias=False, dilation=dilation)
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        self.conv1 = nn.Conv3d(2, self.inplanes, kernel_size=[7,7,1], stride=[2,2,1], padding=[3,3,0],
                               bias=False)
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilate=False):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                conv1x1(self.inplanes, planes * block.expansion, stride=[2,2,1]),
                norm_layer(planes * block.expansion),
            )

        layers = []
        layers.append(block(self.inplanes, planes, stride=[2,2,1], downsample=downsample, groups=self.groups,
