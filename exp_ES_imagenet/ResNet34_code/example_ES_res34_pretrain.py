...
# 使用以下命令直接执行
# CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6 python -m torch.distributed.launch --nproc_per_node=7 example_ES_res34_pretrain.py
...
master = False 
save_folder = 'LIAF_from_ANN_34_reconstruct'
...
train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size,pin_memory=True,drop_last=True,sampler=train_sampler)
...
#checkpoint = torch.load('./LIAF_from_ANN_34/0.pkl', map_location=torch.device('cpu'))
checkpoint = torch.load('./LIAF_warmup_on_imagenet_34_reconstruct/pre_warmup_0.pkl', map_location=torch.device('cpu'))
...
for epoch in range(num_epochs):
    ...
    if master:
        ...
        if acc > bestacc:
            bestacc = acc
            print('===> Saving models...')
            torch.save(snn.module.state_dict(),'./'+save_folder+'/'+str(int(bestacc))+'.pkl')
