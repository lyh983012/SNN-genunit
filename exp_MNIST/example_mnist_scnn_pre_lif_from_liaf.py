...
save_folder = 'mnist_scnn_pre_lif_from_liaf'
writer = SummaryWriter(comment='../runs/mnist_scnn_pre_lif_from_liaf')

#todo: input your data path
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
names = 'mnist_scnn_pre_lif_from_liaf'
...
snn = modules.LIAFCNN(config)
snn.load_state_dict(torch.load('/home/lyh/pre_train/exp_N_MNIST/MNIST_LIAF/99LIAFmodelsaved.t7'))
snn.to(device)
...
for epoch in range(num_epochs):
   ...
   #evaluation
    ...
    if acc > best_acc:
        ...
        print('===> Saving models...')
        torch.save(snn.state_dict(), workpath+'/'+save_folder+'/'+str(int(best_acc))+'modelsaved.t7')
        print('===> Saved')
        print("best:",best_acc)
